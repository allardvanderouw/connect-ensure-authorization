...
const { start, stop } = require('./start-custom-server');
...
describe('ensure custom integration test', () => {
  before((done) => {
    start()
      .then(() => {
        done();
    ...
    it('should throw an error if the user is not authorized', (done) => {
      ...
      login()
        ...
        .then((createScopeResponse) => {
          assert.equal(createScopeResponse.status, 418);
          assert.deepStrictEqual(createScopeResponse.body, { message: 'I\'m a teapot' });
